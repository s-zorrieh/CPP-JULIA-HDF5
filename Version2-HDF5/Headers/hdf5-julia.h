...
static bool PRINTING_EVALUATION_COMMANDS = 0;

void SET_PRINTING_STATUS(bool TorF) {
    ::PRINTING_EVALUATION_COMMANDS = TorF;
}
...
class JuliaCommand
{
    ...
    public:
        ...
        jl_value_t *evaluate(){
            if (::PRINTING_EVALUATION_COMMANDS)
                std::cout
            ...
            } else if (::PRINTING_EVALUATION_COMMANDS){
                std::cout << "Done!\n\n";
            }
...
//            uint8_t publishnnb, datasourceid, algoid;
            void operator++ (){++INDEX; jl_gc_enable(1);build_constructor();}
            bool operator==(Row other) {return (other.get_index() == static_cast<size_t>(INDEX.getInt()));}
            void set_index(size_t i){INDEX.update(static_cast<unsigned int>(i));}
        private:
            ...
            void build_constructor() {
                Accessing = (pds + INDEX.getIndexStr());
...
class HDF5File
{
    public:
        class iterator: std::iterator<
                ...
                Row
                >{
                Row current_row{1, "bcm1fagghist"};

            public:
                explicit iterator(size_t index) : current_row(index, "bcm1fagghist") {}
                iterator& operator++() {++current_row; return *this;}
                reference operator*() const {return current_row;}
                bool operator==(iterator other) {return other.current_row == current_row;}
                bool operator!=(iterator other) {return !((*this) == other);}
        };

        HDF5File(std::string FileName, std::string vname, std::string Path = "", std::string Mode = "r", bool store = false) {
            Varname = "HDF5_" + vname;
            name = FileName;
            pathof = Path;
            mode = Mode;
            dir = pathof + FileName;
            dataSetName = Varname + "bcm1fagghist";
        ...
        iterator end() {return iterator(length());}
